...
import { Shield, ArrowLeft, Download, User, Tag, HardDrive, Cpu, Package, Eye, Star } from "lucide-react";
...
export default function PluginDetail() {
  ...
  const fetchReviews = () =>
    db.entities.PluginReview.filter({ plugin_id: id }, "-created_date", 50).then(setReviews);

  useEffect(() => {
    Promise.all([
      db.entities.Plugin.filter({ id }).then((r) => r[0] ?? null),
      db.entities.PluginReview.filter({ plugin_id: id }, "-created_date", 50),
    ]).then(([pluginData, reviewData]) => {
      setPlugin(pluginData);
      setReviews(reviewData);
      setLoading(false);
      // Increment view count
      if (pluginData) {
        db.entities.Plugin.update(pluginData.id, { views: (pluginData.views ?? 0) + 1 });
      }
    });
  }, [id]);
  ...
  const handleDownload = () => {
    if (!plugin) return;
    db.entities.Plugin.update(plugin.id, { downloads: (plugin.downloads ?? 0) + 1 });
  ...
  if (!plugin) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        <p className="text-muted-foreground font-body">Plugin not found.</p>
        <Link to="/plugins" className="text-primary font-body text-sm hover:underline">← Back to Plugins</Link>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-6 pt-28 pb-20">
        ...
        >
          ...
          <div className="absolute bottom-6 left-6 flex items-center gap-3">
            <span className={`px-2.5 py-1 rounded-full text-xs font-body font-semibold border ${categoryColors[plugin.category] ?? "bg-muted text-muted-foreground border-border"}`}>
              {plugin.category}
            </span>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-body text-muted-foreground mb-4">
                ...
                {plugin.rating_count > 0 && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>{(plugin.rating ?? 0).toFixed(1)} ({plugin.rating_count} reviews)</span>
                  </div>
                )}
              </div>
              ...
              ">
                <ReactMarkdown
                  components={{
                    p: ({ children }) => <p className="font-body text-muted-foreground leading-relaxed mb-4 whitespace-pre-line">{children}</p>,
                  }}
                >{plugin.long_description || plugin.description}</ReactMarkdown>
              </div>
            </div>
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-foreground mb-6">
                Reviews
                {reviews.length > 0 && <span className="text-muted-foreground font-body text-base font-normal ml-2">({reviews.length})</span>}
              </h2>
          ...
          >
            ...
            <div className="rounded-2xl bg-card border border-primary/30 p-6 flex flex-col gap-4">
              ...
              <a
                href={plugin.download_url && plugin.download_url !== "#" ? plugin.download_url : undefined}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {plugin.tags.map((tag) => (
                        <span key={tag} className="px-2 py-0.5 rounded-full bg-secondary text-xs font-body text-secondary-foreground border border-border">
                          {tag}
                        </span>
                      ))}
                    </div>
