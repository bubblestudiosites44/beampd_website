...
export default function PluginLogin({ onAuth }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let account;
      if (mode === "login") {
        account = await logIn(form.username, form.password);
      } else {
        ...
        >
          ...
          <div className="rounded-2xl bg-card border border-border p-8">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                <label className="text-xs font-body text-muted-foreground uppercase tracking-wider mb-1.5 block">Username</label>
                <input
                  type="text"
                  value={form.username}
              ...
              {mode === "signup" && (
                <div>
                  <label className="text-xs font-body text-muted-foreground uppercase tracking-wider mb-1.5 block">Email</label>
                  <input
                    type="email"
                    value={form.email}
                    onChange={set("email")}
              ...
              <div>
                <label className="text-xs font-body text-muted-foreground uppercase tracking-wider mb-1.5 block">Password</label>
